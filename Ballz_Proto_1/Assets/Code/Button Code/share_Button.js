...
//
// THIS IS THE BUTTON CODE FOR THE SHARE SCREEN
// THE CAMERA WILL PAN TO  WHATEVER WE HAVE FOR SHARE
//
...
function OnMouseDown(){
	...
	camReset();
	chooseLocation = 1;
	newPosition = GameObject.Find("share").transform.position;
}
...
function Update () {
	...
	if(chooseLocation == 1){
		...
		if(GameObject.Find("Main Camera").transform.position == previousPosition){
			Debug.Log("FINISHED - Share Button");
			chooseLocation = 0;
		}
	}
...
function camReset(){
	Debug.Log("CamReset Entered - share button");
